...
def load_comment(lang):
    res = {}
    for idx in [1, 2]:
        input_path = "dbp15k/" + lang + "/comment_%d"%idx
...
def write_gcn_dataset(lang, vec, train, dev, test, sent_dict, out_dir, topk=200):
    ref = np.array(loadfile("dbp15k/"+lang+"/ref_ent_ids", 2))
    Ls = ref[:,0]
    Rs = ref[:,1]
    Lvec = vec[Ls]
    Rvec = vec[Rs]
...
if __name__ == "__main__":
    ...
    train = np.array(loadfile("dbp15k/"+args.lang+"/train", 2))
    dev = np.array(loadfile("dbp15k/"+args.lang+"/dev", 2))
    test = np.array(loadfile("dbp15k/"+args.lang+"/ref_ent_ids", 2)) # to get all desc embeddings
